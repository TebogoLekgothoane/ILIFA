...
export default function TrailsScreen() {
  return (
    <ScreenShell>
      ...
      <View style={styles.section}><SectionHeading title="Your progress" /><View style={styles.progressCard}><View style={styles.progressTop}><Text style={styles.progressTitle}>East London Heritage Trail</Text><Text style={styles.progressValue}>3 / 8</Text></View><View style={styles.progressTrack}><View style={styles.progressFill} /></View><Text style={styles.progressCopy}>Three places discovered. Keep walking.</Text></View></View>
      <SectionHeading title="More ways to explore" />
      <TrailMini title="Rail & Industry" meta="2.4 km · 5 places" icon="truck" />
      <TrailMini title="Coastal Stories" meta="3.1 km · 6 places" icon="wind" />
    </ScreenShell>
  );
}

function TrailMini({ title, meta, icon }: { title: string; meta: string; icon: keyof typeof Feather.glyphMap }) {
  return <Pressable style={styles.miniRow}><View style={styles.miniIcon}><Feather name={icon} size={18} color={ui.accent} /></View><View style={{ flex: 1 }}><Text style={styles.miniTitle}>{title}</Text><Text style={styles.miniMeta}>{meta}</Text></View><Feather name="arrow-up-right" size={17} color={ui.mutedForeground} /></Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  miniRow: { flexDirection: 'row', alignItems: 'center', gap: 13, paddingVertical: 13, borderBottomWidth: 1, borderBottomColor: ui.border },
  miniIcon: { width: 42, height: 42, borderRadius: 15, backgroundColor: '#201D3B', alignItems: 'center', justifyContent: 'center' },
